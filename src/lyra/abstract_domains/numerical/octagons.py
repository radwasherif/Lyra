...
class ElinaLattice(Lattice):
    ...
    def __init__(self, dim: int, idx_to_varname: dict, varname_to_idx: dict):
        self.dim = dim
        self.idx_to_varname = idx_to_varname
        self.varname_to_idx = varname_to_idx
    ...
    def copy(self):
        copy = ElinaLattice(self.dim, deepcopy(self.idx_to_varname), deepcopy(self.varname_to_idx))
    ...
    def add_dim(self, var_name: str):
        ...
        dimchange.contents.dim[0] = self.dim
        ...
        self.varname_to_idx[var_name] = self.dim
    ...
    def make_input(self, dim: int, pp: int) -> 'ElinaLattice':
        new_elina = self.copy()
        input_varname = f"id{pp}"
        if not input_varname in new_elina.varname_to_idx:
            new_elina.add_dim(input_varname)

        dimperm = elina_dimperm_alloc(new_elina.dim)
        for i in range(new_elina.dim):
            dimperm.contents.dim[i] = i
        input_idx = new_elina.varname_to_idx[input_varname]
        dimperm.contents.dim[dim] = input_idx
        dimperm.contents.dim[input_idx] = dim
        new_elina.abstract = elina_abstract0_permute_dimensions(new_elina.man, False, new_elina.abstract, dimperm)
        new_elina.project(dim)
        new_elina.equate()
        self.project(dim)
        self.equate()
        return new_elina
    ...
    def to_json(self):
        js = dict()
        js["dim"] = self.dim
        js["idx_to_varname"] = self.idx_to_varname
        js["varname_to_idx"] = self.varname_to_idx
        if self.is_bottom():
            js["lincons_array"] = "⊥"
        if self.is_top():
            js["lincons_array"] = "T"
        ...
        for i in range(self.lincons_array.size):
            ...
            for j in range(lincons.linexpr0.contents.size):
                ...
                lincons_json["signs"].append(1 if elina_coeff_equal_int(coeff, 1) else -1)
    ...
    def check_input(self, id, line_number, start_offset, end_offset, input_value, id_value, id_input_line):
        for i in range(self.lincons_array.size):
            ...
            for j in range(lincons.linexpr0.contents.size):
                ...
                term_val = None
                varname = self.idx_to_varname[linterm.dim]
                # if this constraint contains a non-input value, just ignore it
                if not varname.startswith("id"):
                    ignore_constraint = True
                    break
                if varname == f"id{id}":
                    term_val = input_value * coeff
                else:
                    map_idx = int(varname[2:])
                    if isinstance(id_value[map_idx], InputError):
                        error = deepcopy(id_value[map_idx])
                        error.display_line = line_number
                        error.start_offset = start_offset
                        error.end_offset = end_offset
                        return error
                    term_val = id_value[map_idx]
                val += term_val
            if ignore_constraint:
                continue
            constant = lincons.linexpr0.contents.cst.val.scalar.contents.val.dbl
            if not (term_val >= constant):
                return InputError(code_line=id, input_line=line_number, start_offset=start_offset, end_offset=end_offset, message=f"must satisfy the relation(s): {self.lincons_to_string(lincons, id_input_line)}")
...
class OctagonState(State):
    ...
    def __init__(self, variables: List[VariableIdentifier]):
        ...
        self.elina = ElinaLattice(dim=len(idx_to_varname), idx_to_varname=idx_to_varname, varname_to_idx=self.varname_to_idx)
    ...
    def forget_variable(self, variable: VariableIdentifier, pp: int):
        ...
        element = self.elina.make_input(dim, pp)
    ...
    def flatten_expr(self, expr: Expression, sign: int):
        ...
        """
        if isinstance(expr, Identifier) or isinstance(expr, Literal):
            return [expr], [sign]
    ...
    def from_json(js):
        idx_to_varname = dict()
        for k, v in js["idx_to_varname"].items():
            idx_to_varname[int(k)] = v
        elina = ElinaLattice(js["dim"], idx_to_varname, js["varname_to_idx"])
        ...
        for lincons in js["lincons_array"]:
            elina.add_linear_constr(lincons["variables"], lincons["signs"], lincons["constant"])
        return elina
